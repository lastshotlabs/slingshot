...
- **AI** - `@lastshotlabs/slingshot-ai`: provider-neutral LLM generation, structured output, tool calling (framework-run loop with per-iteration spend re-entry), moderation, spend/cost accounting. Providers: `anthropic`, `openai`, `grok`, `deepseek`, `gemini`, `openai-compatible` (Ollama/vLLM/LM Studio/OpenRouter — free local inference is a config change)
