...
`getActorTenantId(c)` is the tenant the actor *belongs to*. `getRequestTenantId(c)` is
...
<Aside type="caution">
  Calling `attachContext` more than once on the same app with a different context throws.
  The framework holds context references via WeakMap and overwriting causes hard-to-debug
  bugs across plugin DI tables.
</Aside>
