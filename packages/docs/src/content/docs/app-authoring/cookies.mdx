...
Signed cookies are *not* encrypted — anyone with the cookie can read the original
...
<Aside type="tip">
  CSRF only matters for cookie auth. Bearer-token and M2M requests bypass the check —
  they're inherently CSRF-safe because attackers can't make a browser forge an
