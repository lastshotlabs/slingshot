...
<Aside type="caution">
  Never compare secrets with `===`. `===` short-circuits at the first byte mismatch and
  leaks the matching prefix length to a remote attacker who can measure response times
  precisely. `timingSafeEqual` runs in constant time regardless of where the values
  diverge.
</Aside>
