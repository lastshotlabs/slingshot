...
<Aside>
  Routes discovered via `routesDir` mount **after** package routes. If you need to intercept before
  package routes (e.g., a global guard), add a plugin with `setupMiddleware` instead.
</Aside>
