...
<Aside type="tip">
  `disconnect()` is called on graceful shutdown. ioredis-style clients should use `disconnect()`
  over `quit()` because subscribe-mode clients hang on `quit()` waiting for unrelated commands.
</Aside>
