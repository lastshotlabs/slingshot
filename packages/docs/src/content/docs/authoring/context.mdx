...
[App Authoring: Context and Request Model](/app-authoring/context-and-request-model/).
...
**Passing tenant identity to asynchronous work.** Capture a
`TenantExecutionContextSnapshot` at the request boundary and restore it inside
the callback with `withTenantExecutionContext(...)`. Snapshots are immutable,
versioned values; they avoid module-global tenant state and preserve identity
across queue, event, job, and realtime boundaries.

## See also

- [Plugin Interface](/authoring/plugin-interface/) — lifecycle phases and when to access context
- [Internals: Context and Registrar](/internals/context-registrar/) — deep dive into the WeakMap implementation
- [Testing Plugins](/authoring/testing-plugins/) — `ctx.clear()` in test setup
