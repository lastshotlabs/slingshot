...
[Core Features: Events and the Event Bus](/slingshot/app-authoring/events-and-the-event-bus/).
...
BullMQ is the built-in option. Other brokers still work through the registry-backed custom event-bus path. See [Custom Event Bus](/slingshot/examples/custom-event-bus/) for the full configuration.
