...
Register your safe events in `setupPost`:
...
const plugin: SlingshotPlugin = {
  name: 'notes',
  setupPost({ bus }) {
    bus.registerClientSafeEvents(['note.created', 'note.updated', 'note.deleted']);
  },
};
...
Forbidden namespaces are never forwarded regardless of registration:
