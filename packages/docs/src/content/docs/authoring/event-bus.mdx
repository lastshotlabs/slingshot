...
[Core Features: Events and the Event Bus](/app-authoring/events-and-the-event-bus/).
...
BullMQ is the built-in option. Other brokers still work through the registry-backed custom event-bus path. See [Custom Event Bus](/examples/custom-event-bus/) for the full configuration.

## Transactional delivery

Ordinary `events.publish()` and `bus.emit()` remain immediate. Applications that
need a domain write and its event to commit atomically can opt into
`events.reliability` with a PostgreSQL or SQLite store. Transactional publication
uses the existing governed envelope and an explicit `delivery: 'outbox'` plus
framework-owned transaction scope.

An enabled outbox requires an event bus implementing the separate
`AcknowledgedEventBus.publishEnvelope()` capability. The framework rejects an
in-process bus, a missing selected SQL store, and unsafe lease/retry settings
during bootstrap before opening reliability infrastructure. This feature
provides at-least-once delivery; consumers must deduplicate by the stable event
ID.

```typescript
await services.transactions.run('postgres', async transaction => {
  const orders = services.entities.get(Order, { scope: transaction });
  ...
  services.events.publish(
    ...
    { orderId: order.id },
    {
      requestTenantId: order.tenantId,
      delivery: 'outbox',
      transaction,
    },
  );
...
The returned envelope is synchronous, but its insert is registered as pending
transaction work. Commit waits for the insert; rollback removes both the domain
write and outbox row. Missing, retained, forged, closed, and wrong-store scopes
fail before outbox SQL.

## See also

- [Internals: Event Bus](/internals/event-bus/) - `InProcessAdapter`, transport contract, and `SlingshotEventMap` extension
- [Custom Event Bus](/examples/custom-event-bus/) - BullMQ plus registry-backed custom buses
- [Realtime with SSE](/examples/realtime-sse/) - forwarding client-safe events to browsers
