...
[`definePackage`](/package-first/define-package/) — pick by the shape of what
you're building, not by perceived level. See
[Composing an App](/composing-an-app/#when-to-use-which) for the decision guide.
...
- [SlingshotContext](/authoring/context/) - reading and writing instance-scoped runtime state
- [CoreRegistrar](/authoring/registrar/) - the registrar contract and what it publishes
- [Event Bus](/authoring/event-bus/) - emitting and subscribing to events
- [Internals: Plugin Lifecycle](/internals/plugin-lifecycle/) - deep dive into boot ordering and phase execution
- [Observability & Distributed Tracing](/guides/observability/) - plugin lifecycle spans and `createChildSpan` for request-level tracing
