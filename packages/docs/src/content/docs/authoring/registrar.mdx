...
| User resolver                                  | `setUserResolver()`       | `slingshot-auth`                 | `getUserResolver(ctx)` or `ctx.userResolver` |
...
import { getContext, getRouteAuth, getUserResolver } from '@lastshotlabs/slingshot-core';
...
const plugin: SlingshotPlugin = {
  ...
  setupPost({ app }) {
    ...
    const userResolver = getUserResolver(ctx);
    void userAuth;
    void userResolver;
  },
};
...
const plugin: SlingshotPlugin = {
  name: 'example',
  setupPost({ config }) {
    config.registrar.setUserResolver({
...
- Capabilities the framework consumes (user resolver, cache adapters)
