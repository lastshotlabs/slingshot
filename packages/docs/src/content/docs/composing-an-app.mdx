...
happens through Package Contracts (capabilities + narrowed entity refs) and events (decoupled
side effects) — never through direct imports between packages.
...
- "I want to teach the entity system a new operation pattern" → entity executor override, not a plugin (see [Lower-level Seams](/slingshot/package-first/lower-level-seams/)).
...
4. **[Package Contracts](/slingshot/package-first/contracts/)** — declare a package's typed
   public surface so other packages can consume its capabilities and entities through a
   provider-owned, boot-validated contract.
...
Once you have an app composed, follow the journey into per-route concerns
([Authoring Routes](/slingshot/authoring-routes/)), data deep dives, security, realtime,
operations, and production. Two reference pages live at the end of the journey for when
you need to step beneath the primary path:

- **[Capabilities and entityRef](/slingshot/package-first/capabilities-and-entity-ref/)** — the underlying mechanism that Package Contracts wrap. Reach for it directly only when a contract is overkill or unavailable.
- **[Lower-level Seams](/slingshot/package-first/lower-level-seams/)** — overrides and manual wiring inside the package model when generated behavior needs tweaking.

## Where the rest of the docs go

This section answers "how do I compose an app." From here, the canonical journey continues
through the rest of the sidebar in order:

1. **[Authoring Routes](/slingshot/authoring-routes/)** — every concern that applies to a single route: validation, DTO mapping, auth/permissions, idempotency, OpenAPI, exception handling.
2. **[Working with Data](/slingshot/core-features/data-and-entities/)** — the entity system in depth: `defineEntity`, operations, storage adapters, generated routes.
3. **[Security](/slingshot/core-features/auth/)** — actor identity, sessions, then permissions on top.
4. **[Realtime](/slingshot/core-features/realtime/)** — Server-Sent Events and WebSockets.
5. **[Operations](/slingshot/core-features/jobs-and-orchestration/)** — durable background work and multi-step workflows.
6. **[Production](/slingshot/guides/production-readiness/)** — deployment, runtime selection, secrets, scaling.
