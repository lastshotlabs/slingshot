...
- "I want to teach the entity system a new operation pattern" → entity executor override, not a plugin (see [Lower-level Seams](/package-first/lower-level-seams/)).
...
1. **[App Config](/app-authoring/app-config/)** — what `defineApp(...)` accepts and
   how the runtime discovers `app.config.ts`.
2. **[Packages](/package-first/define-package/)** — declare a feature module with
   its own entities, routes, middleware, and dependencies.
3. **[Domains and Routes](/package-first/domain-and-route/)** — typed custom routes
   inside a package, for endpoints that don't map to entity CRUD.
4. **[Package Contracts](/package-first/contracts/)** — declare a package's typed
...
5. **[Events and the Event Bus](/app-authoring/events-and-the-event-bus/)** — the
   shared transport for cross-package side effects, SSE, and webhook delivery.
6. **[Plugin Interface](/authoring/plugin-interface/)** — the `SlingshotPlugin`
...
([Authoring Routes](/authoring-routes/)), data deep dives, security, realtime,
...
- **[Capabilities and entityRef](/package-first/capabilities-and-entity-ref/)** — the underlying mechanism that Package Contracts wrap. Reach for it directly only when a contract is overkill or unavailable.
- **[Lower-level Seams](/package-first/lower-level-seams/)** — overrides and manual wiring inside the package model when generated behavior needs tweaking.
...
1. **[Authoring Routes](/authoring-routes/)** — every concern that applies to a single route: validation, DTO mapping, auth/permissions, idempotency, OpenAPI, exception handling.
2. **[Working with Data](/core-features/data-and-entities/)** — the entity system in depth: `defineEntity`, operations, storage adapters, generated routes.
3. **[Security](/core-features/auth/)** — actor identity, sessions, then permissions on top.
4. **[Realtime](/core-features/realtime/)** — Server-Sent Events and WebSockets.
5. **[Operations](/core-features/jobs-and-orchestration/)** — durable background work and multi-step workflows.
6. **[Production](/guides/production-readiness/)** — deployment, runtime selection, secrets, scaling.
