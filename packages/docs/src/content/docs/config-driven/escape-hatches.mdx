...
Full examples: [Override a generated route](/entity-system/generated-routes-overrides-and-extra-routes/#override-a-generated-route)
...
Full examples: [Add an extra route](/entity-system/generated-routes-overrides-and-extra-routes/#add-an-extra-route)
...
Full examples: [File-based routes](/config-driven/routes/#file-based-routes-with-routesdir)
...
Full examples: [Factories wiring mode](/entity-system/storage-and-adapter-wiring/#factories)
...
It is also the correct mode when operation methods are supplied externally: standard factories
require every `op.custom` declaration to include a factory for the active backend.

Full examples: [Manual wiring mode](/entity-system/storage-and-adapter-wiring/#manual)
...
Full examples: [Composite factories](/entity-system/storage-and-adapter-wiring/#composite-factories)

### Storage backend swapping

Switch the backing store for entities at the app root without changing entity definitions or
adapter code. Memory, SQLite, Postgres, Redis, and Mongo expose one factory surface, while the
generated [entity backend support matrix](/reference/entity-backend-support/) records which
semantics each store actually proves.

Full examples: [Backend swapping at the app root](/entity-system/storage-and-adapter-wiring/#backend-swapping-at-the-app-root)
...
Framework transaction scopes intentionally expose no raw client, query method, Drizzle transaction,
SQLite handle, or Mongo session. A custom repo that needs a driver-owned transaction must keep that
contract private and must not pretend it participates in `transactions.run()` unless it is wired
through the framework's scoped adapter resolver. Never capture an unscoped adapter before
`run()` and assume it becomes scoped.
...
Full examples: [System fields, storage fields, and conventions](/entity-system/define-entity/#system-fields-storage-fields-and-conventions)
...
Full examples: [Package domain routes](/package-first/domain-and-route/)
...
Full examples: [Config-Driven Domain example](/examples/config-driven-domain/#build-the-plugin)
...
Full examples: [Plugin Interface](/authoring/plugin-interface/)
...
- [Generated Routes, Overrides, and Extra Routes](/entity-system/generated-routes-overrides-and-extra-routes/) — full examples for overrides and extra routes
- [Storage and Adapter Wiring](/entity-system/storage-and-adapter-wiring/) — full examples for all wiring modes
- [defineEntity](/entity-system/define-entity/) — system fields, storage fields, and conventions
- [Package Domain Routes](/package-first/domain-and-route/) — typed routes that belong to the package
- [Routes](/config-driven/routes/) — file-based routes and the route decision matrix
- [Adapters and Factories](/config-driven/adapters/)
- [Cross-Entity and Transactions](/config-driven/cross-entity/)
- [Packages and Capabilities](/config-driven/packages/)
