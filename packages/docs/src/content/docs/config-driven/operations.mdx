...
Operations are plain JSON config objects with a `kind` field. The framework reads those objects
and generates typed adapter methods and HTTP routes. Standard CRUD comes from the entity definition
itself; operations extend it with domain-specific behavior. Backend wiring is capability-gated, so
use the generated [entity backend support matrix](/reference/entity-backend-support/) instead of
assuming every operation has identical semantics on every store.
...
Startup rejects an operation/backend pairing when the selected standard adapter cannot provide
the required guarantee. Composite transaction rollback is currently supported by SQLite and Postgres;
memory, MongoDB, and Redis fail startup for `op.transaction`.

SQLite routes declarative and package-service transactions through one per-app FIFO coordinator.
While a `BEGIN IMMEDIATE` scope is open, unrelated standard entity CRUD, list, clear,
initialization, and named operations wait for that scope to commit or roll back. They cannot
accidentally execute on the shared connection inside another request's transaction.

Release CI emits `entity-conformance.v2.json`. Its transaction guarantee catalog requires the
same declarative commit/rollback, scoped two-entity commit/rollback, nesting, mismatch, and
closed-adapter cases to pass on both SQLite and PostgreSQL. Weakening or removing one of those
claims makes report validation fail.

</Aside>

<Aside type="caution" title="Redis safety boundary">
  Standard Redis wiring proves strict primary-key create, CRUD, TTL visibility, filtering, sorting,
  cursor traversal, and its supported read operations against a live Redis service. It rejects
  secondary or compound uniqueness, transition, increment, array mutation, consume, upsert, atomic
  batch/computed aggregate, and composite rollback before opening the Redis client. Those guarantees
  stay unsupported until one complete atomic Lua or `WATCH` implementation covers the whole write
  lifecycle and passes the shared race cases.
</Aside>
...
A `param:` field whose value the caller **omits** drops that constraint entirely
rather than matching null — so one operation serves both "all of them" and
"just these". Passing `null` explicitly still matches null-valued records.
...
listByTarget: op.lookup({
  fields: {
    ...
    value: 'param:value', // omit → every value; pass one → filter to it
  },
  returns: 'many',
});
```

:::caution[An added field changes the route path]
A lookup's default route appends one `:param` segment per entry in `fields`, so
adding an optional field silently moves the URL and 404s existing callers. Pin
the path when the new field should be a **query** param instead:

```typescript
routes: {
  operations: {
    listByTarget: { path: 'list-by-target/:targetId/:targetType' },
  },
}
```

Query parameters are merged into the operation's params at request time, so
`?value=🔥` reaches the same matcher without appearing in the path.
:::

### Bounding a `returns: "many"` lookup

Callers may pass a `limit` (as a query parameter or in the body). It is clamped
to the entity's `pagination.maxLimit`, and the result's `hasMore` reports
whether more rows matched.

```http
GET /community/reactions/list-by-target/abc/thread?value=🔥&limit=6
```

**Omitting `limit` means unbounded, and that is deliberate.** Callers exist that
must see every matching row to be correct — an aggregate that re-reads all of a
target's rows to recompute a denormalized total would silently under-count
behind a default page size. Ask for a bound when you want one.
...
Atomically add to (or subtract from) a numeric field on a specific record where the selected backend
claims that guarantee. Postgres uses `SET field = field + $n`, Mongo uses `$inc`, and memory
serializes the mutation. Standard Redis wiring rejects `increment` because its JSON
read-modify-write path is not atomic.
...
Standard wiring requires a factory for the active backend and fails startup when one is absent.
If your application mixes an operation method onto an adapter itself, use manual adapter wiring;
`op.custom({ http: ... })` is not a route-only marker in standard factories.
...
## Schema-operation rollout

Field configuration also carries explicit migration intent. When a persisted
field changes name, declare the source so generation emits a rename instead of
an ambiguous remove/add pair:

```typescript
displayName: field.string({ renameFrom: 'name' });
```

The source must exist only in the previous v2 snapshot. A rename that changes
type must also name an operator-reviewed `migrationTransform`; otherwise
planning fails before emitting SQL. Review the resulting phased plan with
`slingshot migrate plan`, run `slingshot migrate verify`, and pass the exact
printed `--approve` digest for contract or destructive work. See
[Migration Engine v2](/guides/migration-engine-v2/) for the ledger and recovery
workflow.
...
- [Entities](/config-driven/entities/)
- [Routes](/config-driven/routes/)
- [Cross-Entity and Transactions](/config-driven/cross-entity/)
