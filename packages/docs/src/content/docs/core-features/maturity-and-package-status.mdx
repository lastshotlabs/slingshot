...
The committed `package-maturity.json` declaration is authoritative. CI
generates and byte-compares the [package maturity evidence table](/core-features/generated-maturity/),
release summary, and runtime warning metadata from that declaration and its
versioned evidence manifests. Hand-written status lists are not release inputs.
