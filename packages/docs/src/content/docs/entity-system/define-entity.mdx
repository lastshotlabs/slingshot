...
Indexes tell the storage backend to optimize queries on specific fields. Configured uniqueness is
validated against the selected store at startup; consult the generated
[entity backend support matrix](/reference/entity-backend-support/) before relying on a backend
switch:
...
See [Route Policy](/entity-system/route-policy/) for the full set of per-route options.
...
- [Optimistic Concurrency](/entity-system/optimistic-concurrency/) — prevent lost updates with version guards and ETags
- [Operations](/entity-system/operations/) — add domain actions like publish, lookup, search
- [Route Policy](/entity-system/route-policy/) — full auth, permissions, rate limits, and event config
- [Storage and Adapter Wiring](/entity-system/storage-and-adapter-wiring/) — swap backends without changing entity code
