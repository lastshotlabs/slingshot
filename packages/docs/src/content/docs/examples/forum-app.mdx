...
<Tabs>
  <TabItem label="Manifest">
    ...
    export async function communityAuthBridge(c: Context, next: Next) {
      ...
      if (userId) c.set('communityPrincipal', { subject: userId, roles });
      await next();
    }
    ```
...
Community middleware reads `communityPrincipal` — not raw auth state. The bridge maps `authUserId` and `roles` (set by `slingshot-auth`) into that shape. This keeps the packages independent: community doesn't depend on auth internals, and auth doesn't know about community.
