...
Returns 200 whenever the process is running. Sufficient for load balancer health checks.

```typescript title="src/routes/health.ts"
...
For Kubernetes readiness probes or staged deployments, verify downstream dependencies before the instance accepts traffic:

```typescript title="src/routes/ready.ts"
import { createRouter, getRedisFromApp } from '@lastshotlabs/slingshot';

const router = createRouter();

declare function getSlingshotApp(c: import('hono').Context): object;
declare function pingPostgres(app: object): Promise<void>;

router.get('/ready', async c => {
  const app = getSlingshotApp(c);

  const checks: Record<string, 'ok' | 'error'> = {};

  try {
    await pingPostgres(app);
    checks.postgres = 'ok';
  } catch {
  ...
  try {
    const redis = getRedisFromApp(app);
...
Register this plugin in `createServer` before other plugins — it has no setup dependencies.
...
`createServer` returns the `app` instance. Register `onError` after startup to wrap Slingshot's own error handling.
...
Slingshot propagates `requestId` through the request context. For span-level distributed tracing, integrate OpenTelemetry:
...
OpenTelemetry is the common choice here: start your tracer provider before `createServer()`, then add the HTTP and database instrumentations that match your runtime and drivers.
