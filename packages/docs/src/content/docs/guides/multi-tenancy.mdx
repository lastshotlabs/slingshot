...
The framework calls your `tenantResolver` on every request before any routes run. The returned string becomes `c.get('tenantId')`, available in all middleware and route handlers. Plugins read it automatically.
...
No extra config needed. They read `c.get('tenantId')` from the request context.
...
With tenant roles enabled, `c.get('tenantRoles')` holds the user's roles scoped to `c.get('tenantId')`.
...
```typescript
tenantResolver: c => {
  // After auth middleware sets authUserId, resolve the user's current org
