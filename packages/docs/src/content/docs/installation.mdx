...
On Bun, `createServer` without a `runtime` argument uses `@slingshot/runtime-bun` automatically.
...
- **Node.js** 20+ — only if using the Node.js runtime adapter (`@slingshot/runtime-node`).
