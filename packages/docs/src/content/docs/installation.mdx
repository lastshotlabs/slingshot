...
On Bun, `createServer` without a `runtime` argument uses `@lastshotlabs/runtime-bun` automatically.
...
- **Node.js** 20+ — only if using the Node.js runtime adapter (`@lastshotlabs/runtime-node`).
