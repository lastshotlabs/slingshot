...
When used through `createOrchestrationPackage()`, the plugin starts and shuts down the adapter for
...
with `createTemporalOrchestrationWorker()` so workflow code and activity handlers can poll the
