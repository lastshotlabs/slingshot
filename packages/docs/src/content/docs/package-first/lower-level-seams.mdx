...
[Plugin Interface](/authoring/plugin-interface/) — that is a parallel first-class
...
| Direct lifecycle control (async bootstrap, teardown) | `SlingshotPlugin` ([Plugin Interface](/authoring/plugin-interface/)) |
...
See [Generated Routes, Overrides, and Extra Routes](/entity-system/generated-routes-overrides-and-extra-routes/) for full details.
...
See [Storage and Adapter Wiring](/entity-system/storage-and-adapter-wiring/) for the factories and manual wiring modes.
...
full [Plugin Interface](/authoring/plugin-interface/) reference.
...
See [Plugin Interface](/authoring/plugin-interface/) for the full lifecycle and
[Composing an App](/composing-an-app/#when-to-use-which) for the package vs plugin
decision guide.
...
[Plugin Interface](/authoring/plugin-interface/) instead. That's a different tier,
not a later step in the same progression.

## Next

- [Plugin Interface](/authoring/plugin-interface/) — `SlingshotPlugin` contract for framework-level plugins
- [Entity System](/entity-system/) — the entity authoring and customization story
- [Examples](/examples/) — complete apps that combine packages, entities, and custom routes
