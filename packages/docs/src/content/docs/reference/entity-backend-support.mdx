...
| `concurrency.version-update` | ✅ Supported | ❌ Version concurrency is not implemented for SQLite yet. | ❌ Version concurrency is not implemented for PostgreSQL yet. | ❌ Version concurrency is not implemented for MongoDB yet. | ❌ Redis version concurrency requires an atomic Lua or WATCH/MULTI implementation. |
| `concurrency.version-delete` | ✅ Supported | ❌ Version concurrency is not implemented for SQLite yet. | ❌ Version concurrency is not implemented for PostgreSQL yet. | ❌ Version concurrency is not implemented for MongoDB yet. | ❌ Redis version concurrency requires an atomic Lua or WATCH/MULTI implementation. |
