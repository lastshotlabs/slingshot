...
export function createSlowModeGuardMiddleware(deps: {
  ...
  return async (c, next) => {
    ...
    const latest = messages.items
      ...
      .reduce<
        (typeof messages.items)[number] | null
      >((candidate, message) => (!candidate || new Date(message.createdAt).getTime() > new Date(candidate.createdAt).getTime() ? message : candidate), null);
    ...
    if (latestAt > 0) {
      ...
      if (remaining > 0) {
        return c.json({ error: `Slow mode — try again in ${remaining}s` }, 429, {
