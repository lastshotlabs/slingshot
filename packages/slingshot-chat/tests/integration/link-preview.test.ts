...
function createMockEmbeds(transform?: (url: string) => MockEmbedData | null): {
  unfurl(urls: string[]): Promise<MockEmbedData[]>;
} {
  return {
    async unfurl(urls: string[]) {
      return urls
        .map(url => transform?.(url) ?? { url, title: `Preview for ${url}`, type: 'link' })
        .filter((e): e is MockEmbedData => e !== null);
    },
  };
}
...
describe('Link preview attachment', () => {
  it('attaches embeds when the message body contains a URL', async () => {
    ...
    const embed = updated?.embeds?.[0] as MockEmbedData | undefined;
  ...
  it('does not attach embeds when the body has no URLs', async () => {
    ...
    expect(updated?.embeds == null || (updated.embeds as unknown[]).length === 0).toBe(true);
  });
  ...
  it('attachEmbeds can be called directly on the message adapter', async () => {
    ...
    expect(updated?.embeds).toHaveLength(1);
    expect((updated?.embeds?.[0] as { url: string })?.url).toBe('https://example.com');
  });
});
