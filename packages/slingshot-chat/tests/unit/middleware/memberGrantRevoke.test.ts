...
describe('memberGrantRevoke', () => {
  test('revokes active room grants after successful membership deletion', async () => {
    ...
    const permissionsAdapter = {
      getGrantsForSubject: async () => [{ id: 'g1', effect: 'allow' }, { id: 'g2', effect: 'allow', revokedAt: new Date() }],
      revokeGrant: async (id: string) => { revoked.push(id); return true; },
    } as unknown as PermissionsAdapter;
    const memberAdapter = { getById: async () => ({ id: 'm1', userId: 'u1', roomId: 'r1' }) } as unknown as RoomMemberAdapter;
    const app = new Hono();
    app.use('*', async (c, next) => { setVar(c, 'actor', { id: 'owner', kind: 'user', tenantId: null }); await next(); });
    app.use('/members/:id', createMemberGrantRevokeMiddleware({ permissionsAdapter, memberAdapter, tenantId: null }));
