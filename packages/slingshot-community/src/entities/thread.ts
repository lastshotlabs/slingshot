...
export const Thread = defineEntity('Thread', {
  ...
  routes: {
    ...
    permissions: {
      ...
      // match what every `requires:` clause and `permissionEvaluator.can(...)`
      // call passes as the action argument. The evaluator does a literal
      // `actions.includes(action)` lookup against the role's action list — bare
      // verbs here would silently never match any operation's `requires:`
      // string and every authed call would 403.
      actions: [
        ...
        'community:container.manage-settings',
        'community:container.manage-automod',
        'community:container.pin',
        'community:container.lock',
        'community:container.delete-content',
        'community:container.review-report',
        'community:container.review-audit',
        'community:container.warn-user',
        'community:container.apply-ban',
        'community:container.lift-ban',
      ],
      roles: {
        owner: ['*'],
        member: ['community:container.read', 'community:container.write'],
        moderator: [
          'community:container.read',
          'community:container.write',
          'community:container.pin',
        ...
        'report-reviewer': ['community:container.review-report'],
        'community-admin': ['*'],
      },
    },
