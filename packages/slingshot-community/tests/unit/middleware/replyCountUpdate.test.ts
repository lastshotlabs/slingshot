...
interface ThreadAdapterSpy {
  incrementReplyCount(id: string): Promise<void>;
  updateLastActivity(
    match: { id: string },
    data: { lastActivityAt?: string; lastReplyById?: string; lastReplyAt?: string },
  ): Promise<void>;
  calls: Array<{ method: string; id: string; data?: unknown }>;
}

function buildSpy(): ThreadAdapterSpy {
  const calls: ThreadAdapterSpy['calls'] = [];
  return {
    calls,
...
function buildApp(spy: ThreadAdapterSpy, responseBody: unknown, status: 201 | 400 | 500 = 201) {
  const app = new Hono();
...
describe('replyCountUpdate middleware', () => {
  ...
  test('updates lastActivity on successful reply creation', async () => {
    ...
    const data = call!.data as { lastReplyById: string; lastReplyAt: string };
  ...
  test('uses current time when createdAt is absent from response', async () => {
    ...
    expect(call).toBeDefined();
    const data = call!.data as { lastActivityAt: string };
