...
export function createNotificationBuilder(
  ...
  async function notify(input: NotifyInput): Promise<NotificationRecord | null> {
    ...
    const createInput: Record<string, unknown> = {
      ...
      // EVERY DEFAULTED NON-NULL COLUMN MUST BE WRITTEN EXPLICITLY HERE.
      //
      // This builder writes THROUGH THE ADAPTER (`notifications.create` /
      // `dedupOrCreate`), not through the entity route — and the route layer is
      // what applies a field's declared `default`. So on Postgres each such
      // column, NOT NULL with no database-level default, received NULL and the
      // insert failed:
      //
      //   null value in column "read" of relation "slingshot_notifications"
      //   violates not-null constraint
      //
      // NO NOTIFICATION COULD BE CREATED AT ALL on Postgres. It survived
      // unnoticed because SQLite accepts the same insert, and dev runs on
      // SQLite while production runs on Postgres — so the feature was dead
      // exactly where it mattered, and nowhere else. Found 2026-07-27 by
      // sgforum asserting a reply-notification end to end.
      //
      // The full set, discovered one failed insert at a time: `read`, `seen`
      // and `createdAt`. `dispatched` and `priority` were already set above,
      // and `id` is generated by the adapter. If a new defaulted column is
      // added to the Notification entity it has to be added here too — the
      // entity declaration alone does NOT cover this write path.
      read: false,
      seen: false,
      createdAt: now,
    };
