...
export function createSchedulingFns(
  ...
  async function schedule(
    ...
    cron: string,
    input: unknown,
  ): Promise<{ id: string; target: { type: 'task' | 'workflow'; name: string }; cron: string; input: unknown }> {
    await ensureStarted();
    const queue =
      target.type === 'task' ? getQueueForTaskName(target.name) : state.workflowQueue;
  ...
  async function unschedule(scheduleId: string): Promise<void> {
    await ensureStarted();
    for (const queue of [state.defaultTaskQueue, state.workflowQueue, ...state.namedQueues.values()]) {
