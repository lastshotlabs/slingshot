...
describe('sqlite crash recovery', () => {
  sqliteTest(
    'recovers an interrupted workflow and completes it on adapter restart',
    async () => {
      ...
      rawDb
        .prepare(`UPDATE orchestration_steps SET status = 'running', completed_at = NULL WHERE run_id = ?`)
        .run(handle1.id);
      rawDb.close();
  ...
  sqliteTest(
    'preserves step attempt count across crash recovery',
    async () => {
