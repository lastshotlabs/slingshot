...
describe('route registration', () => {
  ...
  test('GET /orgs returns 200 for admin', async () => {
    expect((await h.app.request('/orgs', { headers: { 'x-user-id': h.adminId } })).status).toBe(
      200,
    );
  });
  ...
  test('POST /orgs/:orgId/members returns 201', async () => {
    const org = (await (
      await h.app.request('/orgs', {
        method: 'POST',
        headers: headers(),
        body: JSON.stringify({ name: 'M Org', slug: 'm-org' }),
      })
    ).json()) as { id: string };
    const res = await h.app.request(`/orgs/${org.id}/members`, {
      method: 'POST',
      headers: headers(),
      body: JSON.stringify({ userId: h.memberId, role: 'member' }),
    });
    expect(res.status).toBe(201);
  });
  ...
  test('POST /orgs/:orgId/invitations returns 201', async () => {
    const org = (await (
      await h.app.request('/orgs', {
        method: 'POST',
        headers: headers(),
        body: JSON.stringify({ name: 'I Org', slug: 'i-org' }),
      })
    ).json()) as { id: string };
    const res = await h.app.request(`/orgs/${org.id}/invitations`, {
      method: 'POST',
      headers: headers(),
  ...
  test('GET /groups returns 200 for admin when group management is enabled', async () => {
    expect((await h.app.request('/groups', { headers: { 'x-user-id': h.adminId } })).status).toBe(
      200,
    );
  });
});
...
describe('custom mount path', () => {
  test('routes respond at configured prefix and are absent at root', async () => {
    const h = await setupOrgPluginHarness({
      mountPath: '/api',
    ...
    try {
      expect(
        (await h.app.request('/api/orgs', { headers: { 'x-user-id': h.adminId } })).status,
      ).toBe(200);
      expect((await h.app.request('/orgs', { headers: { 'x-user-id': h.adminId } })).status).toBe(
        404,
      );
...
describe('middleware composition', () => {
  ...
  test('member receives 403 on admin-only POST /orgs', async () => {
    const res = await h.app.request('/orgs', {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-user-id': h.memberId },
  ...
  test('admin passes through admin guard on POST /orgs', async () => {
    const res = await h.app.request('/orgs', {
      ...
      body: JSON.stringify({ name: 'Working Org', slug: 'working' }),
    });
    expect(res.status).toBe(201);
  });

  test('invite lookup route is public (auth: none)', async () => {
    const res = await h.app.request('/orgs/dummy/invitations/lookup', {
      method: 'POST',
