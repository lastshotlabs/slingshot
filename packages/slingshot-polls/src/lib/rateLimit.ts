...
import { getActorId, getRequestTenantId } from '@lastshotlabs/slingshot-core';
...
export function parseDuration(input: string): number {
  ...
  switch (match[2]) {
    ...
    default:
      throw new Error(`[slingshot-polls] Unknown unit: ${match[2]}`);
  }
}
...
export function buildRateLimitMiddleware(
  ...
  return async (c, next) => {
    ...
    if (perTenant) {
      const tenantId = getRequestTenantId(c) ?? undefined;
      if (tenantId) {
        const result = await backend.check(
