...
export function createPollsPackage(
  ...
  // Long-lived Proxy view published through `PollsRuntimeCap`. Constructed
  ...
  // runtime is wired.
  const runtimeTarget = Object.create(null) as PollsPluginState;
  const runtimeView: PollsPluginState = new Proxy<PollsPluginState>(runtimeTarget, {
    get(_target, property) {
      if (typeof property === 'symbol' || property === 'then') return undefined;
      if (!runtimeStateRef) {
        throw new Error(
          `[slingshot-polls] runtime.${String(property)} accessed before setupPost completed; resolve PollsRuntimeCap from setupPost or later.`,
        );
      }
  ...
  return definePackage({
    ...
    capabilities: {
      provides: [
        ...
        // setupPost has run.
        provideCapability(PollsRuntimeCap, () => runtimeView),
      ],
    },
