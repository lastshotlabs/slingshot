...
export interface SsrRouteMatch {
  ...
  /**
   * Optional hook supplied by an `SsrRouteSource` to override how this match's
   * route module is loaded.
   *
   * The default behavior — used by the file-based source and any source that
   * leaves this undefined — is to dynamically import `filePath` and treat the
   * resulting module as already conforming to slingshot's canonical shape
   * (`{ load, meta?, default: Page }`).
   *
   * Sources whose route files use a different export shape (e.g. a TanStack
   * adapter that reads `{ Route, ssr }`) populate this field so the executor
   * adapts the module to the canonical shape on each request.
   *
   * The function MUST return an object with `load`, optional `meta`, and
   * `default` (the page component). Caching is the source's responsibility.
   *
   * Renderers should access this via `executeRouteModule` rather than calling
   * it directly — the executor handles the fallback to the default importer
   * and propagates loader signals correctly.
   */
  readonly loadModule?: () => Promise<{
    readonly load: import('./types').DefineRouteOptions<Record<string, unknown>>['load'];
    readonly meta?: import('./types').DefineRouteOptions<Record<string, unknown>>['meta'];
    readonly default: import('./types').DefineRouteOptions<Record<string, unknown>>['Page'];
  }>;
}
...
export interface SsrPluginConfig {
   ...
   *
   * Optional when `routeSource` is supplied — the source becomes the source of
   * truth for routes in that case. If both are set, `routeSource` wins and
   * `serverRoutesDir` is ignored. If neither is set, plugin construction throws.
   */
   ...
   * routes from a different system — e.g. a TanStack Router file tree, a
   * custom config object, an in-memory test fixture.
   *
   * When omitted, the plugin constructs a file-based source from
   * `serverRoutesDir` (the historical behavior — backwards-compatible).
   */
  readonly routeSource?: import('./routeSource/types').SsrRouteSource;
  /**
