...
import { clampDeliveryTimeoutMs, TIMEOUT_CLAMP_MS } from '../../src/plugin.js';
...
describe('clampDeliveryTimeoutMs (P-WEBHOOKS-9)', () => {
  test('passes through values at or below 120s', () => {
    ...
    expect(clampDeliveryTimeoutMs(TIMEOUT_CLAMP_MS, { deliveryId: 'd', endpointId: 'e' }, bus)).toBe(
      TIMEOUT_CLAMP_MS,
    );
  ...
  test('clamps oversized values, logs warning, and emits webhook:timeoutClamped', () => {
    ...
    const result = clampDeliveryTimeoutMs(
      200_000,
      { deliveryId: 'd1', endpointId: 'ep1' },
      bus,
    );
